...
const TitleHandler = {
    ...
    async handle(handlerInput) {
        ...
        return handlerInput.responseBuilder
            .speak(
                `OK je ${state} le titre ${podcastnumber + 1} <break time="1s"/> ${
...
const SynopsisHandler = {
    ...
    async handle(handlerInput) {
        let podcastnumber =
            handlerInput.requestEnvelope.request.intent.slots.SynTitle.value - 1;
        return handlerInput.responseBuilder
            .speak(
...
const LastHandler = {
    canHandle(handlerInput) {
        ...
        return (
            request.type === "IntentRequest" && request.intent.name === "LastIntent"
        );
    },
    async handle(handlerInput) {
        ...
        return handlerInput.responseBuilder
            .speak(
                `Le dernier podcast joué était le titre ${i + 1} : ${podcasts[i].name}`
            )
            .withSimpleCard(
                "Dernier Podcast",
...
const ListHandler = {
    canHandle(handlerInput) {
        ...
        return (
            request.type === "IntentRequest" && request.intent.name === "ListIntent"
        );
    },
    async handle(handlerInput) {
        var gist = await getGist(getGithubToken(handlerInput));
